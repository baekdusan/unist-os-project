                    ...
                    thread_func *function, void *aux) // thread create 시 초기화 되는 것들. 초기 priority도 이때 받아서 설정되네.
{
  ...
  //wait exec을 위한 초기화들
  t -> parent = thread_current(); //init_thread에 있게되면, 
  //해당 function에서 blocked로 바뀐 순간에 thread_current()를 실행해서 assertion error발생함.
  t -> exit_status = NULL;
  list_push_back(&thread_current() -> child_list, &t -> child_elem);
  sema_init(&t -> wait_sema, 0);  //wait syscall에서 사용.
...
#ifdef USERPROG
  process_exit();
  sema_up(&(thread_current() -> wait_sema)); //부모가 자식 종료를 기다리고 있음.
#endif
