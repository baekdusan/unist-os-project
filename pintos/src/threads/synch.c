...
void sema_down(struct semaphore *sema)
{
  ...
  while (sema->value == 0)
  {                                                                                     // sema를 모두 가져가서 대기해야함.//sema_wait list같은 경우는 thread_elem을 넣고 있어서 priority_large만들어둔걸로 사용 가능.
...
void sema_up(struct semaphore *sema)
{
  ...
  // 위 if문에서는 정렬 후 unblock했으므로 unblock된 스레드의 순서 미정; running thread를 비교해야함.
  intr_set_level(old_level);
}
...
void priority_donation(struct thread *cur)
{
  ...
  while (cur->wait_lock != NULL)
  { // cur이 대기중인 lock이 존재하면.
...
void lock_acquire(struct lock *lock)
{
  ...
  if (!thread_mlfqs)
  { // mlfq mode일 때는 donation을 위한 것들 필요 X.
    if (lock->holder != NULL)
    { // 현재 lock을 누군가 들고있으면!
...
{ // 내가 볼 때, 이걸 호출하는 thread와 연관된거 싹 reassign해줘야 할 것 같은데..
...
void cond_wait(struct condition *cond, struct lock *lock) // lock을 가지 실행됨.
{
...
void cond_signal(struct condition *cond, struct lock *lock UNUSED)
{
  ...
  { // conditional variable에 waiter가 존재하면
