...
struct thread // thread structure있는 곳.
{
   ...
   struct file *exec_file; //현재 실행중인 파일. load에서 file_deny_write()에서 사용.
   struct list_elem child_elem; //parent가 child가질 때 사용하는 것
   struct list child_list;  //이 list하나만 있으면 head ptr이런것도 필요 없을듯.
   struct thread *parent; //부모 쓰레드 wait을 위해서 필요함.
   struct semaphore wait_sema; //자식 종료 대기시 사용. load되는거 기다려야함.
