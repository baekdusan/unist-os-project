...
syscall_init (void) 
{
  ...
  sema_init(&(thread_current() -> exec_sema), 0); //syscall들어왔을 때 다시 초기화 해줘야함.
}
...
int mywrite(int fd, const void *buffer, unsigned size){
    ...
    if(fd < 2 || fd >= 64 || cur->file_descriptor[fd] == NULL){ //fd가 2 아래거나, 64이상이거나, 그리고 쓰려는 것에 NULL이면 오류.
        lock_release(&global_lock);
        myexit(-1);
        return -1;
    }
...
int myread(int fd, void *buffer, unsigned size){
    ...
    if(fd < 2 || fd >= 64){ //file descriptor범위를 넘어섬.
        lock_release(&global_lock);
        return -1;
    }
...
void myclose(int fd){
    ...
    if(cur -> exec_file == cur -> file_descriptor[fd]){
      cur -> exec_file = NULL;
    }
...
int myexec(const char *cmdline){
    ...
    if(pid == TID_ERROR){ //execute 실패시 TID_ERR로 옴.
        return -1;
    }
    struct list_elem *e;
